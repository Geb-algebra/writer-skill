...
- SCQA の役割は、本論の要約ではなく問題設定である。導入を読んだだけで記事の論点・定義・理由・具体例・方法論が先取りできる状態にしてはいけない。
- S, C, Qは必要十分に書く。読者が記事を読む動機と問いを理解できる量に留め、本文で扱う詳細を前出ししない。
- Situation は、読者が既に知っている状況だけを書く。本文で主張する新しい見方・定義・分類・判断基準は置かない。
- Complication は、その状況から自然に生じる困りごとだけを書く。Answerを支える理由・具体例・方法論は置かない。
- Question は、記事が答える問いだけを書く。問いの中で、本文の結論や根拠を説明しない。
- Answerは与えられたメインメッセージをそのまま使用する。 Answerは簡潔であるべきなので、メインメッセージ以外は含めない。 Answerを支える詳細はContentに配置する。
- 用語の厳密な定義、具体例、方法、分類、判断基準、Answerを支える理由は原則Contentに配置する。
- Introductionでは、本文で独自定義する語を絶対に使わない。導入でその概念が必要な場合は、独自定義語を出さず、その語が指す意味を読者が既に理解できる普通の表現で直接書く。
- 本文で独自定義する語の初出はContentに置く。定義・背景・具体例・近い概念との差分もContentで説明する。
...
この原則は、Introductionを本文のミニ版にするための指示ではない。
IntroductionはSCQAの役割を優先し、本論の詳細を含めない。
最終稿に必要な定義・理由・具体例・方法論・分類・判断基準は、IntroductionではなくContentに十分な情報量で配置する。
